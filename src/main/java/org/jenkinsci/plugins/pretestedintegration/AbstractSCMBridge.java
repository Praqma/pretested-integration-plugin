...
public abstract class AbstractSCMBridge implements Describable<AbstractSCMBridge>, ExtensionPoint {
    ...
    public static List<SCMBridgeDescriptor<?>> getDescriptors() {
        List<SCMBridgeDescriptor<?>> list = new ArrayList<>();
    ...
    /**
     * Method that determines if we should prepare workspace for integration. If
     * not we throw the nothing to do exception
     *
     * @param build
     * @param listener
    ...
    /**
     * Override this to associate an integrated commit with a pointer with the
     * starting point before merge. This is used to roll back in case of
     * integraion failure
     *
     * @param build
     * @param launcher
