...
public class PretestCommitStatus implements RootAction {
	...
	static boolean looselyMatches(URI notifyUri, String repository) {
		...
		try {
			...
			result = Objects.equal(notifyUri.getScheme(),
				...
				&& Objects.equal(notifyUri.getQuery(),
				repositoryUri.getQuery());
		} catch ( URISyntaxException ex ) {
			LOGGER.log(Level.SEVERE, "could not parse repository uri "
					+ repository, ex);
		}
		return result;
	}
	...
	public HttpResponse doNotifyCommit(
			@QueryParameter(required=true) final String url)
	...
	private HttpResponse handleNotifyCommit(URI url)
			throws ServletException, IOException {
		...
		for (AbstractProject<?,?> project
			...
			LOGGER.log(Level.INFO, "Triggering the polling of {0}",
	...
	private static final Logger LOGGER =
			Logger.getLogger(PretestCommitStatus.class.getName());
	/*
