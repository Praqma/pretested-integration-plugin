...
/**
 * <h3>Test that examines the behaviour and the data produced by 'Multiple SCM plugin'</h3>
 * <p>Created this test to check out the behaviour of how the GitSCM interacts with
 * 'Multiple SCM plugin'.</p>
 * 
 * <p>We see that every time we create one build we get two build actions. Each pointing
 * to the same revision.</p>
 * 
 * <p>This test was created to check the behaviour when running MultiSCM plugin with
 * git.</p>
 * 
 * @author Mads
 */
public class MultipleScmIT {
    ...
    public void verifyOrderOfBuildDataObjects() throws Exception {
        ...
        /**
         * Now we try something different. Trigger both in same polling interval. We get two new branches.
         * 
         */        
